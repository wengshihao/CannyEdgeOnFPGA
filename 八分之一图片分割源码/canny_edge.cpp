...
void canny_edge(wide_stream* in_stream1, wide_stream* out_stream1,
		...
		wide_stream* in_stream5, wide_stream* out_stream5,
		wide_stream* in_stream6, wide_stream* out_stream6,
		wide_stream* in_stream7, wide_stream* out_stream7,
		wide_stream* in_stream8, wide_stream* out_stream8,
...
#pragma HLS INTERFACE axis port=in_stream5 bundle=INPUT5
#pragma HLS INTERFACE axis port=out_stream5 bundle=OUTPUT5
#pragma HLS INTERFACE axis port=in_stream6 bundle=INPUT6
#pragma HLS INTERFACE axis port=out_stream6 bundle=OUTPUT6
#pragma HLS INTERFACE axis port=in_stream7 bundle=INPUT7
#pragma HLS INTERFACE axis port=out_stream7 bundle=OUTPUT7
#pragma HLS INTERFACE axis port=in_stream8 bundle=INPUT8
#pragma HLS INTERFACE axis port=out_stream8 bundle=OUTPUT8
...
#pragma HLS INTERFACE ap_stable port=threshold2
	...
	GRAY_IMAGE src_bw5(rows, cols);
	GRAY_IMAGE src_bw6(rows, cols);
	GRAY_IMAGE src_bw7(rows, cols);
	GRAY_IMAGE src_bw8(rows, cols);

	GRAY_IMAGE src_blur1(rows, cols);
	GRAY_IMAGE src_blur2(rows, cols);
	GRAY_IMAGE src_blur3(rows, cols);
	GRAY_IMAGE src_blur4(rows, cols);
	GRAY_IMAGE src_blur5(rows, cols);
	GRAY_IMAGE src_blur6(rows, cols);
	GRAY_IMAGE src_blur7(rows, cols);
	GRAY_IMAGE src_blur8(rows, cols);
	...
	GRAY_IMAGE src51(rows, cols);
	GRAY_IMAGE src52(rows, cols);
	GRAY_IMAGE src61(rows, cols);
	GRAY_IMAGE src62(rows, cols);
	GRAY_IMAGE src71(rows, cols);
	GRAY_IMAGE src72(rows, cols);
	GRAY_IMAGE src81(rows, cols);
	GRAY_IMAGE src82(rows, cols);
	...
	GRAY_IMAGE_16S sobel_gx5(rows, cols);
	GRAY_IMAGE_16S sobel_gy5(rows, cols);
	GRAY_IMAGE_16S sobel_gx6(rows, cols);
	GRAY_IMAGE_16S sobel_gy6(rows, cols);
	GRAY_IMAGE_16S sobel_gx7(rows, cols);
	GRAY_IMAGE_16S sobel_gy7(rows, cols);
	GRAY_IMAGE_16S sobel_gx8(rows, cols);
	GRAY_IMAGE_16S sobel_gy8(rows, cols);
    ...
    GRAY_IMAGE_16 grad_gd5(rows, cols);
    GRAY_IMAGE_16 grad_gd6(rows, cols);
    GRAY_IMAGE_16 grad_gd7(rows, cols);
    GRAY_IMAGE_16 grad_gd8(rows, cols);
    ...
    GRAY_IMAGE_16 suppressed5(rows, cols);
    GRAY_IMAGE_16 suppressed6(rows, cols);
    GRAY_IMAGE_16 suppressed7(rows, cols);
    GRAY_IMAGE_16 suppressed8(rows, cols);
    ...
    GRAY_IMAGE thresholded5(rows, cols);
    GRAY_IMAGE thresholded6(rows, cols);
    GRAY_IMAGE thresholded7(rows, cols);
    GRAY_IMAGE thresholded8(rows, cols);
    ...
    GRAY_IMAGE canny_edges5(rows, cols);
    GRAY_IMAGE canny_edges6(rows, cols);
    GRAY_IMAGE canny_edges7(rows, cols);
    GRAY_IMAGE canny_edges8(rows, cols);
...
#pragma HLS PIPELINE enable_flush rewind
		...
		ap_uint<32> dat5 = in_stream5->data;
		src_bw5.write(GRAY_PIXEL(dat5.range(7,0)));
		src_bw5.write(GRAY_PIXEL(dat5.range(15,8)));
		src_bw5.write(GRAY_PIXEL(dat5.range(23,16)));
		src_bw5.write(GRAY_PIXEL(dat5.range(31,24)));
		++in_stream5;

		ap_uint<32> dat6 = in_stream6->data;
		src_bw6.write(GRAY_PIXEL(dat6.range(7,0)));
		src_bw6.write(GRAY_PIXEL(dat6.range(15,8)));
		src_bw6.write(GRAY_PIXEL(dat6.range(23,16)));
		src_bw6.write(GRAY_PIXEL(dat6.range(31,24)));
		++in_stream6;

		ap_uint<32> dat7 = in_stream7->data;
		src_bw7.write(GRAY_PIXEL(dat7.range(7,0)));
		src_bw7.write(GRAY_PIXEL(dat7.range(15,8)));
		src_bw7.write(GRAY_PIXEL(dat7.range(23,16)));
		src_bw7.write(GRAY_PIXEL(dat7.range(31,24)));
		++in_stream7;

		ap_uint<32> dat8 = in_stream8->data;
		src_bw8.write(GRAY_PIXEL(dat8.range(7,0)));
		src_bw8.write(GRAY_PIXEL(dat8.range(15,8)));
		src_bw8.write(GRAY_PIXEL(dat8.range(23,16)));
		src_bw8.write(GRAY_PIXEL(dat8.range(31,24)));
		++in_stream8;
	}
	...
	hls::Duplicate( src_bw5, src51, src52 );
	hls::Duplicate( src_bw6, src61, src62 );
	hls::Duplicate( src_bw7, src71, src72 );
	hls::Duplicate( src_bw8, src81, src82 );
    ...
    hls::Sobel<1,0,3>( src51, sobel_gx5 );
    hls::Sobel<0,1,3>( src52, sobel_gy5 );
    hls::Sobel<1,0,3>( src61, sobel_gx6 );
	hls::Sobel<0,1,3>( src62, sobel_gy6 );
	hls::Sobel<1,0,3>( src71, sobel_gx7 );
	hls::Sobel<0,1,3>( src72, sobel_gy7 );
	hls::Sobel<1,0,3>( src81, sobel_gx8 );
	hls::Sobel<0,1,3>( src82, sobel_gy8 );
	...
	gradient_decomposition( sobel_gx5, sobel_gy5, grad_gd5 );
	gradient_decomposition( sobel_gx6, sobel_gy6, grad_gd6 );
	gradient_decomposition( sobel_gx7, sobel_gy7, grad_gd7 );
	gradient_decomposition( sobel_gx8, sobel_gy8, grad_gd8 );
	...
	nonmax_suppression( grad_gd5, suppressed5 );
	nonmax_suppression( grad_gd6, suppressed6 );
	nonmax_suppression( grad_gd7, suppressed7 );
	nonmax_suppression( grad_gd8, suppressed8 );
	...
	hysteresis( suppressed5, canny_edges5, threshold1, threshold2);
	hysteresis( suppressed6, canny_edges6, threshold1, threshold2);
	hysteresis( suppressed7, canny_edges7, threshold1, threshold2);
	hysteresis( suppressed8, canny_edges8, threshold1, threshold2);
...
#pragma HLS PIPELINE enable_flush rewind
		for(int c = 0; c < col_packets; c++){
			...
			ap_uint<32> dat5;
			dat5.range(7,0) = canny_edges5.read().val[0];
			dat5.range(15,8) = canny_edges5.read().val[0];
			dat5.range(23,16) = canny_edges5.read().val[0];
			dat5.range(31,24) = canny_edges5.read().val[0];
			out_stream5->data = dat5;
			out_stream5->user = (r == 0 && c == 0)? 1: 0;
			out_stream5->last = (r == rows-1 && c == col_packets-1)? 1: 0;
			++out_stream5;

			ap_uint<32> dat6;
			dat6.range(7,0) = canny_edges6.read().val[0];
			dat6.range(15,8) = canny_edges6.read().val[0];
			dat6.range(23,16) = canny_edges6.read().val[0];
			dat6.range(31,24) = canny_edges6.read().val[0];
			out_stream6->data = dat6;
			out_stream6->user = (r == 0 && c == 0)? 1: 0;
			out_stream6->last = (r == rows-1 && c == col_packets-1)? 1: 0;
			++out_stream6;

			ap_uint<32> dat7;
			dat7.range(7,0) = canny_edges7.read().val[0];
			dat7.range(15,8) = canny_edges7.read().val[0];
			dat7.range(23,16) = canny_edges7.read().val[0];
			dat7.range(31,24) = canny_edges7.read().val[0];
			out_stream7->data = dat7;
			out_stream7->user = (r == 0 && c == 0)? 1: 0;
			out_stream7->last = (r == rows-1 && c == col_packets-1)? 1: 0;
			++out_stream7;

			ap_uint<32> dat8;
			dat8.range(7,0) = canny_edges8.read().val[0];
			dat8.range(15,8) = canny_edges8.read().val[0];
			dat8.range(23,16) = canny_edges8.read().val[0];
			dat8.range(31,24) = canny_edges8.read().val[0];
			out_stream8->data = dat8;
			out_stream8->user = (r == 0 && c == 0)? 1: 0;
			out_stream8->last = (r == rows-1 && c == col_packets-1)? 1: 0;
			++out_stream8;
		}
	}
}
